...
(12) **Completed electrostatic baseline and fabrication constraint.** The baseline device is modeled as a three-gate structure beneath 50 nm of conformal ALD Al₂O₃ in LAr at 87 K [39,62]:

![Baseline three-gate device geometry](../dev_journal/assets/comprehensive_figures/Figure_Electrostatic_Baseline_final.png)
The Al₂O₃ film covers the gate tops, gate sidewalls, and exposed SiO₂ between the gates. The first conservative planning geometry uses 20 µm-wide, 200 µm-long clock gates separated by 20 µm gaps on a 40 µm-pitch. A perimeter guard surrounds the active region, and a separate 100 × 100 µm² electrode provides endpoint collection or induced-charge sensing. The LAr drift gap is 1.00 mm. The baseline top-grid voltage is −50 V, the mean gate potential is +2 V, and the initial clock levels are 0, 2, and 4 V. These are planning values for model release and are not optimized operating voltages. The 50 nm film is the baseline release calculation, $\varepsilon_{r,\mathrm{LAr}} \approx 1.50$ near the normal boiling point [59]. The value $\varepsilon_{r,\mathrm{Al_2O_3}} = 8.0$ is retained as a nominal room-temperature input [44]. Before G1, the finite-element calculation will be repeated using the measured film thickness and a cryogenic dielectric-constant sensitivity range consistent with the documented temperature dependence of ALD Al₂O₃ [60]:

$$
E_{\mathrm{LAr}} = \frac{V_g - V_t}{H + \left(\varepsilon_{\mathrm{LAr}}/\varepsilon_{\mathrm{ox}}\right) t_{\mathrm{ox}}}
$$

for: $V_g = +2\,\mathrm{V}$; $V_t = -50\,\mathrm{V}$; $H = 1.00\,\mathrm{mm}$; $t_{\mathrm{ox}} = 50\,\mathrm{nm}$; $\varepsilon_{r,\mathrm{LAr}} = 1.505$; $\varepsilon_{r,\mathrm{Al_2O_3}} = 8.0$

$$
E_{\mathrm{LAr}} = 5.20 \times 10^{4}\,\mathrm{V/m} = 0.520\,\mathrm{kV/cm}
$$

$$
E_{\mathrm{ox}} = 9.78 \times 10^{3}\,\mathrm{V/m}
$$

and the one-dimensional voltage drop across the 50 nm oxide is approximately 0.489 mV. The applied field drives electrons toward the oxide. It is an approach to surface field, not a demonstrated mechanism for holding free electrons at a finite height. In the planar dielectric approximation, the image contribution for an electron in LAr adjacent to the higher permittivity Al₂O₃ is:

$$
U_{\mathrm{im}}(z) = \frac{e^{2}}{16 \pi \varepsilon_{0} \varepsilon_{\mathrm{LAr}}\, z}\,\frac{\varepsilon_{\mathrm{LAr}} - \varepsilon_{\mathrm{ox}}}{\varepsilon_{\mathrm{LAr}} + \varepsilon_{\mathrm{ox}}}
= -\frac{0.163}{z\,[\mathrm{nm}]}\,\mathrm{eV}
$$

The combined planar potential is:

$$
U_{e}(z) = \mathrm{constant} + e\,E_{\mathrm{LAr}}\,z - \frac{0.163}{z\,[\mathrm{nm}]}\,\mathrm{eV}
$$

Its derivative is positive for $z > 0$, so this baseline has no finite height static minimum. The applied field and idealized image term both drive the electron toward the solid. The continuum expression also becomes physically incomplete at atomic distances because it does not include surface chemistry, dielectric entry, adsorbates, disorder, trap capture, or local electronic structure [11,41,42,46,47,50]. Consequently, all lateral clocking calculations are conditional on the experimentally unresolved existence of a mobile or reversibly localized near-interface population. Stage 1, not the electrostatic model, decides whether that population exists. A transparent lateral proxy represents the 40 μm-pitch gate structure as:

$$
\phi(x, z) = \bar{\phi} - E_{\mathrm{LAr}}\, z + \tilde{V}_{0}\,\exp\!\left(-\frac{2\pi z}{p}\right)\cos\!\left(\frac{2\pi x}{p}\right)
$$

For the conservative 40 µm-pitch planning geometry, the lateral proxy uses $p = 40\,\mu\mathrm{m}$ and $\tilde{V}_0 = 2.0\,\mathrm{V}$, corresponding to a 4 V peak-to-peak surface modulation. At the conditional analysis plane $z = 0.50\,\mu\mathrm{m}$, the attenuation is 0.924, the potential amplitude is 1.85 V, the peak-to-trough electron-energy modulation is 3.70 eV, and the maximum lateral field is 2.90 kV/cm. This calculation demonstrates that the proposed electrode scale can create substantial lateral electrostatic modulation if a mobile or reversibly localized near-interface population exists. It does not demonstrate vertical localization, lateral mobility, or charge-transfer efficiency. The released finite geometry must be evaluated by two-dimensional and three-dimensional finite-element calculations before fabrication.

| Status | Included |
| :---: | :--- |
| Completed | One-dimensional multilayer solution; explicit sign convention; baseline geometry and voltage set; lateral analytic proxy; vertical planar-energy analysis; fixed-charge sensitivity; charge and signal scaling. |
| Preliminary | Existing in-house two-dimensional finite-volume solver; approximately 0.2% centerline agreement with the analytic multilayer limit; provisional finite-gate potential plots. |
| Required before fabrication release | Mesh refinement with less than 5% change in barrier, capacitance, and weighting-potential outputs; finite gate edges and guard; substrate/backplane treatment; measured film thickness; measured or bounded fixed charge; weighting potentials; gate-to-sense capacitances; maximum corner fields; and an independent numerical cross-check [31-33,39,62]. |

### Fixed-charge sensitivity

For a 50 nm Al₂O₃ film, the oxide capacitance per unit area is

$$
\frac{C_{\mathrm{ox}}}{A} = \frac{\varepsilon_{0}\,\varepsilon_{\mathrm{ox}}}{t_{\mathrm{ox}}}
= \frac{(8.854 \times 10^{-12}\,\mathrm{F/m})(8)}{50 \times 10^{-9}\,\mathrm{m}}
= 1.417 \times 10^{-3}\,\mathrm{F/m^{2}}
= 0.1417\,\mu\mathrm{F/cm^{2}}
$$

A fixed sheet charge density $N_f$ produces an oxide-equivalent potential shift given by

$$
|\Delta V_s| \simeq \frac{e\,|N_f|}{C_{\mathrm{ox}}/A}
$$

For the 50 nm Al₂O₃ film, the corresponding potential shifts are:

| Fixed-charge density $N_f$ (cm⁻²) | Potential shift $\Delta V_s$ (V) |
| :---: | :---: |
| $10^{11}$ | 0.113 |
| $10^{12}$ | 1.13  |
| $10^{13}$ | 11.3  |

Thus, $10^{12}\,\mathrm{cm^{-2}}$ is not an appropriate universal accept/reject cutoff. At this oxide thickness, a fixed-charge density of $10^{12}\,\mathrm{cm^{-2}}$ corresponds to a potential shift of approximately 1.13 V, which is already comparable to the assumed 2 V lateral modulation amplitude. A spatially uniform offset may be partly compensated by bias retuning, whereas spatial variation on the gate-pitch scale can distort or eliminate the intended potential wells. If the allocated local potential error is 0.10 V, the corresponding requirement on the spatial variation in effective fixed charge is

$$
\delta N_f < \frac{0.10\,(C_{\mathrm{ox}}/A)}{e} = 8.8 \times 10^{10}\,\mathrm{cm^{-2}}
$$

The gate criterion will therefore use the measured mean effective fixed charge, its uncertainty, hysteresis, leakage, and an experimental or modeled bound on spatial variation. These quantities will be propagated through the actual field solution; no universal fixed charge density cutoff will be imposed [46].
...
| Deadline | Gate and deliverable | Quantitative acceptance and evidence | Scientific redirection if hypothesis fails | Consequence if execution is incomplete |
| :--- | :--- | :--- | :--- | :--- |
| August 28, 2026 | Transition to TMA + revisit any remaining mods on tool | Purchase is made for the appropriate precursor and order is placed by the end of workday | Revisit tool infrastructure and look to purchase a secondary equally comparable TMA/H₂O ALD chemistry (e.g., TiO₂ or HfO₂) | Work closely with vendors and administrative assistant to purchase the next most readily available precursor |
| October 16, 2026 | G0: in-house TMA/H₂O Al₂O₃ qualification | Both precursor saturation curves; cycle-count linearity $R^{2} \ge 0.99$; five consecutive runs with thickness CV $\le 5\%$; within-sample range/mean $\le 5\%$; raw logs, thickness maps, and contamination/sample handling records | Poor films initiate bounded diagnosis, but there is no outsourced deposition bypass around qualification | The dissertation program fails under the explicit statement in paragraph 11 |
| December 18, 2026 | G1: design release readiness | (i) A released layout compliant with documented fabrication limits; (ii) analytic benchmark agreement within 1% and mesh refinement changes below 5%; (iii) measured 50 nm witness-film thickness and an explicitly measured or bounded fixed charge input; (iv) a calibrated $10^{4}$-electron equivalent test injection with measured signal-to-noise ratio; and (v) a source-disabled clock-feedthrough template. | G1 does not test interfacial localization and does not decide whether the proposed mechanism exists. | A missing item triggers a dated corrective action and schedule revision. It does not trigger a LXe redirection. |
| February 26, 2027 | G2: Stage 1 charge arrival, retention, and recovery | At $Q_{\mathrm{inj}} = 10^{4}\,e^{-}$, $Q_{\mathrm{rec}}/Q_{\mathrm{avail}} \ge 10\%$ after a commanded 1 ms hold, with 95% lower confidence bound above the threshold; charge-accounting closure within 20%; two samples and two LAr fills; complete same-run control set | Process variant, anneal/dehydrate/passivate, alternative oxide, non-oxide/cap, dynamic confinement, or quantitative upper-bound chapter. LXe only with a model-based liquid-specific reason | Formal progress review within five business days. Missing calibrated data, controls, or interpretable analysis places continuation of the research path under review |
| April 23, 2027 | G-Materials: material and surface dependence | At least three insulating surface conditions with two samples each; characterized thickness, roughness, chemistry or process monitor; a surface-dependence claim requires a factor-of-two change exceeding combined uncertainty and repeating across samples/fills | If no dependence is resolved, stop open-ended material screening. Carry forward the most stable stack and report a quantitative null result. | New variants freeze. Repeated misses remove materials optimization as a dissertation claim. |
| June 4, 2027 | G3: Stage 2 two-region transfer | $T_{A \to B} \ge 50\%$ with 95% lower confidence bound above 50%; origin loss and destination gain close within 20%; reverse clock reversal; intentional drain disappearance; residual artifact rejection $>5\sigma$; two devices and two fills | Bounded scan of thickness, pitch, overlap, clock amplitude, edge rate, dwell, holding field, and trap release behavior. If no directionality remains, stop Stage 3 and center the dissertation on retention and quantitative transfer limits | Stage 3 is blocked; committee review within one week. An incomplete experiment is formal progress failure |
| August 1, 2027 | G4: Stage 3 multigate transport | At least three boundaries; per-step CTE $\ge 90\%$ with 95% lower confidence bound above 90%; cumulative retention $\ge 72.9\%$; complete origin, destination, drain, delayed release, bulk loss, and unresolved-loss budget | Quantify clock parameters and loss dependence. If directional behavior remains but CTE is low, redirect to short-range trap-limited transport physics; if directionality vanishes, stop multigate claims | Repeated-sensing work is frozen; no additional multigate redesign without a dated committee-approved corrective plan |
| October 31, 2027 | G5: Stage 4 repeated-return discovery test | Same packet completes at least ten sense, move-away, verify-empty, return, and re-sense cycles; mean loss $\le 5\%$ per cycle with 95% upper confidence bound $\le 5\%$; the rms noise of the averaged measurement follows $N^{-1/2}$ within 20% for $N = 1, 4$, and $10$ after correlated-noise correction. | Dedicated floating-gate or differential sensor, RF sensing, separated storage/sense regions, or revised reset sequence. State clearly if transport succeeds but repeated sensing does not | Final scope decision within one week; repeated-sensing claim is removed if the test is not executed. |
| February 29, 2028 | G6: Stage 5 detector-generated charge | Correctly report $4.14 \times 10^{4}$ initial pairs for the 976 keV line, then directly measure arrival charge to within 20%; detector-generated recovery/transfer signature $>5\sigma$; repeat in two independent fills or campaigns [56,57] | If UV packets work but detector charge does not, redirect to recombination, track geometry, drift, purity, and collection coupling | Detector-relevance claims are removed; extrapolation from UV data is not a substitute |
| August 31, 2028 | G7: dissertation and architecture synthesis | Committee-ready evidence chain; source-to-sense charge budget; final material/process table; demonstrated versus proposed functions separated; reproducible raw data and code; CMOS-compatible route | A completed negative result becomes the dissertation result: quantitative bounds on recovery, retention, transfer, surface dependence, and repeated sensing | No new branch without committee approval. Committee determines final continuation or termination milestones from the completed record |
