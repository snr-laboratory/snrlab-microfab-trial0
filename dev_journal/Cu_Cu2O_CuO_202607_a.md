...
## Code for 200/200 updated Sun Jul 19
```
#include <Arduino.h>
#include <SPI.h>
...
/* === Cu / Cu2O / CuO SELECTIVE-PHASE STACK — MEGA + ANALOG SHIELD ============
   REVISION: 20260719 -- 200+200 REPRODUCIBILITY RUN (rev 9).
             [v9] PRESSURE SERIAL STREAMING REMOVED. Pressure (Shield->AN1->IGC)
             is now logged ONLY by dual_logger_v6 in its own terminal. This
             firmware's serial port carries valve/state events + commands ONLY --
             NO pressure spam. Terminal 1 = dual_logger_v6 (pressure). Terminal 2
             = this firmware (control + events). PlatformIO = build/upload only.

             Real water+air run: Phase 1 = 200 cycles H2O (D26+D34 dose),
             Phase 2 = 200 cycles AIR (D24+D36 dose). Identical timing to the
             canonical Run J/K recipe (500ms pulse / 3s dwell / 5s N2 purge /
             3s evac). phase1_dry = FALSE so water actually doses. Purpose:
             reproduce the standard 200/200 recipe on the FIXED gas path
             (water safety valve stops backstreaming) to test film
             reproducibility run-to-run -- the ALD prerequisite.

             CABLE (20260719): AN1 readout now uses purchased cable
             X0033BKF7 (XMSJSIY BNC-male -> DuPont test-lead), extended with
             jumpers and heat-shrunk at the junctions so bare conductors cannot
             touch anything. Replaces the hand-made coax pigtail. Divider
             (10k series / 4.7k shunt, ratio 0.3197) and Shield IN0 tap
             UNCHANGED -- only the cable body between IGC BNC and the divider
             changed. See wiring note below.
   ...
   IGC100 -> DIVIDER -> SHIELD ADC:
     ...
     CABLE X0033BKF7 (20260719): BNC-male mates IGC rear AN1; DuPont-side leads
     carry center -> 10k series -> junction (red jumper to Shield A0) and
     shield/braid -> Shield GND (black jumper) with the 4.7k shunt across A0->GND.
     Extra jumpers add reach; heat-shrink over every splice = no exposed metal.
   ...
   PIN MAP -- MEGA (REV 4, 20260709):
     ...
     D34 -> VALVE_H2O_SAFETY    IN4 / K4 = H2O safety valve (SS-3J-24VDC)
     D36 -> VALVE_AIR_SAFETY    IN5 / K5 = AIR(TMA-line) safety (NVZ110 pilot)

   VALVE PAIRING (enforced -- the two halves ALWAYS fire and clear together):
     H2O pair = D26 (H2O ALD3 dose) + D34 (H2O safety)   -- Phase 1 only
     AIR pair = D24 (AIR ALD dose)  + D36 (AIR safety)    -- Phase 2 only
   Each precursor's dose valve and its safety valve are one logical unit; the
   safety valve catches any vapor the dose valve leaves behind on close. They
   are never opened or closed independently during a run.
...
#define VALVE_H2O_SAFETY  34   // K4 -- SS-3J-24VDC water safety valve
#define VALVE_AIR_SAFETY  36   // K5 -- NVZ110 pilot -> 6LVV-DPFR4 air-line safety


// ---- ANALOG SHIELD -- IGC100 PRESSURE READOUT (boot sanity ping only) -----
...
// [v9] Used ONLY by the one-time boot ADC sanity ping. NOT streamed during runs.
float readPressureTorr(){
  unsigned int adc = analog.read(IGC_CHANNEL);
...
// [v9] pressure serial STREAMING REMOVED. Pressure (Shield -> AN1 -> IGC) is now
// owned entirely by dual_logger_v6 in its own terminal. The firmware NEVER prints
// pressure during a run -- this serial port carries ONLY valve/state events and
// your commands. readPressureTorr() is kept solely for the one-time boot ADC
// sanity ping so a dead cable still shows up in the boot log.
...
// H2O pair = dose (D26) + water safety (D34). Always together.
inline void h2oPairOn (){ relayOn (VALVE_H2O_ALD); relayOn (VALVE_H2O_SAFETY); }
inline void h2oPairOff(){ relayOff(VALVE_H2O_ALD); relayOff(VALVE_H2O_SAFETY); }
// AIR pair = dose (D24) + air safety (D36). Always together.
...
//                    ---- PHASE 1: Cu2O (H2O) ----   ---- PHASE 2: CuO (AIR) ----
//                    pulse dwell purge evac  cyc      pulse dwell purge evac  cyc
// *** 200 water + 200 air, canonical Run J/K timing (500/3000/5000/3000).
...
// [v9] Pressure streaming state REMOVED entirely. dual_logger_v6 owns pressure.
...
// DRY-MOCK flag. TRUE = Phase 1 runs full timing + N2 but H2O valves (D26+D34)
// never open. *** FALSE for today's REAL 200/200 water+air run -- water doses. ***
bool phase1_dry = false;
...
void handleEstop(){
  ...
  k8_manual_refs = 0;
  Serial.println(F("*** E-STOP *** All valves CLOSED (H2O pair D26+D34, AIR pair D24+D36, N2 D28), K8 OFF."));
}
...
void startPhase(Phase_t p){
  ...
  if (p == PH_CU2O){
    ...
    Serial.println(F("AIR phase: AIR pair D24+D36 doses together. N2 purges each cycle."));
  }
  if (cyclesLeft == 0){
...
void handleStartRun(){
  ...
  Serial.println(F("[v9] Pressure is logged separately by dual_logger_v6 (AN1). This"));
  Serial.println(F("     terminal shows valve/state events only -- no pressure spam."));
  Serial.print(F("Phase1 Cu2O cycles: ")); Serial.println(R.cu2o_cycles);
  if (phase1_dry) Serial.println(F(">>> PHASE 1 = DRY MOCK: full timing + N2, NO water dose (D26/D34 stay closed) <<<"));
...
void printMenu(){
  ...
  Serial.println(F("    8/* = D34 (H2O safety)   ON/OFF"));
  ...
  Serial.println(F("  PRESSURE READOUT: handled by dual_logger_v6 (AN1) -- NOT here."));
...
void setup(){
  ...
  pinMode(VALVE_H2O_SAFETY,  OUTPUT);
  ...
  Serial.println(F("Firmware: Cu2O_stack_20260719_200x200_repro (rev 9, REAL water+air, NO pressure spam, cable X0033BKF7)"));
  Serial.println(F("Valve pairs: H2O = D26+D34 (dose+safety), AIR = D24+D36 (dose+safety)."));
  ...
  if (phase1_dry) Serial.println(F("*** phase1_dry = TRUE : Phase 1 runs full timing + N2, NO water dose ***"));
  else            Serial.println(F("Phase1 = Cu2O (H2O, D26+D34, Run J timing). Phase2 = CuO (AIR, D24+D36)."));
  printMenu();
...
void checkInputs(){
  ...
  if (Serial.available()){
    ...
    else if (c == '8'){ if (manualAllowed()){ k8AcquireForValve(); relayOn (VALVE_H2O_SAFETY); Serial.println(F("D34 (H2O safety) ON")); } }
    else if (c == '*'){ if (manualAllowed()){ relayOff(VALVE_H2O_SAFETY); k8ReleaseForValve(); Serial.println(F("D34 (H2O safety) OFF")); } }
...
void loop(){
  ...
  // [v9] No pressure streaming. dual_logger_v6 samples AN1 independently.
  ...
  switch (S){
    ...
    case PULSE_ON:
      if (now - t0 >= main_lead_ms){
        if (phase == PH_CU2O){
          if (phase1_dry){ logEvent(F("DRY MOCK: H2O valves stay CLOSED (no dose)")); }
          else           { h2oPairOn(); logEvent(F("H2O pair D26 + D34 ON")); }
        } else {
          airPairOn(); logEvent(F("AIR pair D24 + D36 ON"));
        }
    ...
    case PULSE_OFF_DWELL:
      if (now - t0 >= curPulse()){
        if (phase == PH_CU2O){
          if (!phase1_dry) h2oPairOff();
          logEvent(F("-> DWELL (precursor residence)"));
        } else {
          airPairOff(); logEvent(F("AIR D24 + D36 OFF -> DWELL (precursor residence)"));
        }
        t0 = now; S = PURGE_ON;
      } break;
    ...
    case PURGE_ON:
      if (now - t0 >= curDwell()){
        ...
        logEvent(F("N2 PURGE ON (all dose pairs CLOSED)"));
    ...
    case DONE:
      if (cyclesLeft > 0 && --cyclesLeft > 0){
        ...
        if (phase == PH_CU2O){
          Serial.println(F("--- PHASE 1 complete -> switching to PHASE 2 (CuO/AIR) ---"));
          safeAll();
          startPhase(PH_CUO);
